 ...
 * Used when browser Notification API is blocked (HTTP/IP) or as an in-app popup.
 */
import {
  createContext,
  useCallback,
...
/** Safe hook — returns no-op when provider missing (SSR / tests). */
...
export function NotificationPopupProvider({ children }: { children: ReactNode }) {
  ...
  const pushPopup = useCallback(
    (n: Omit<AppNotificationPopup, "id"> & { id?: string }) => {
      ...
      setItems((prev) => {
        ...
        // Dedupe same title+body within the visible stack
        if (
          prev.some(
            (x) => x.title === n.title && x.body === n.body,
          )
        ) {
          return prev;
        }
      ...
      window.setTimeout(() => dismiss(id), 12_000);
    },
    [dismiss],
  );
  ...
  return (
    <NotificationPopupContext.Provider value={value}>
      {children}
      <div
        className="pointer-events-none fixed right-3 bottom-3 z-[100] flex w-[min(100%-1.5rem,22rem)] flex-col gap-2 sm:right-5 sm:bottom-5"
        aria-live="polite"
      >
